...
//PASSWORD AND NAME
unsigned char userPass[PASSWORD_SIZE];
char userName[UPATH_SIZE];
...
/*
    При выходе из директории шифруем ее содержимое
*/
...
/*
    Создаем директорию и помещаем туда конфиг с паролем текущего юзера
*/
...
loopback_rename(const char *from, const char *to)
{
    ...
    rename_user_dir(userName,from,to);
    return 0;
}
...
{
    crypt_user_dirs(userName,userPass);
...
int main(int argc, char *argv[])
{
    ...
    switch(ch = getch()) {
        case 'r':
            if(!registration(userName,userPass))
                return 0;
            break;
        case 'l':
